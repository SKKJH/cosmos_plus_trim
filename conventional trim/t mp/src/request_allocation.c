...
//        	buf_cnt++;
        	...
        	if(dataBufMapPtr->dataBuf[bufEntry].prevEntry != DATA_BUF_NONE)
        	{
...
//	static XTime tStart, tEnd, , first_nlb, first_start;
	int tempNlb, tempSlba, blk0, blk1, blk2, blk3;
	unsigned int nlb, slba;
	...
	for (int i=0; i<nr; i++)
	{
		...
		if (((SLICES_PER_SSD * 4) < nlb) || ((SLICES_PER_SSD * 4) < slba))
		{
			err++;
			break;
		}
...
//unsigned int ForcedTRIM (unsigned int trimSizeMB)
//{
//    int TOTAL_BLOCKS = 3145728;
//    int nlb = 262144 / 16;  // 기존 코드대로 nlb 계산
//    int slba = 262144 + (nlb * (og_thres - thres));
//    int loopCount = trimSizeMB / 64;
//
//    while (nlb > 2 && loopCount > 0)
//    {
//        TRIM(slba, 0, 0, 0, 0);
//        slba += 4;
//        nlb  -= 4;
